...
instance Functor m => Functor (WriterT w m) where
    ...
    fmap f writerTArg = 
        let arg = runWriterT writerTArg 
        in WriterT $ fmap (\(a, w) -> (f a, w)) arg
...
instance (Monoid w, Applicative m) => Applicative (WriterT w m) where
    ...
    writerTFunc <*> writerTArg = 
        let func = runWriterT writerTFunc
            arg  = runWriterT writerTArg
            l = \(f, w1) (a, w2) -> (f a, w1 <> w2)
        in  WriterT $ pure l <*> func <*> arg
